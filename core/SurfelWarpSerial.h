...
public:
    ...
    // the method to get m_updated_geometry_index
    int GetGeometryIndex() const { return m_updated_geometry_index; }
    SurfelGeometry::Ptr GetSurfelGeometry() const
    {
        return m_surfel_geometry[m_updated_geometry_index];
    }
...
private:
    ...
    void saveCameraObservations(const CameraObservation& observation,
        const boost::filesystem::path& save_dir);

    // The rendered solver maps, required the same cuda context (but not OpenGL
    // context)
    void saveSolverMaps(const Renderer::SolverMaps& solver_maps,
        const boost::filesystem::path& save_dir);

    // Save the coorresponded geometry and obsertion
    void saveCorrespondedCloud(const CameraObservation& observation,
        unsigned vao_idx, const boost::filesystem::path& save_dir);

    // The rendered and shaded geometry, This method requires access to OpenGL
    // pipeline
    void saveVisualizationMaps(unsigned num_vertex, int vao_idx,
        const Eigen::Matrix4f& world2camera,
        const Eigen::Matrix4f& init_world2camera,
        const boost::filesystem::path& save_dir, bool with_recent = true);

    // The directory for this iteration
    static boost::filesystem::path createOrGetDataDirectory(
