...
#include "core/render/imgui/imconfig.h"
#include "core/render/imgui/imgui.h"
#include "core/render/imgui/imgui_impl_glfw.h"
#include "core/render/imgui/imgui_impl_opengl3.h"
...
private:
    ...
    /* DEAR ImGUI windows related variables and funtions
     */
private:
    void initImGui();
...
public:
    void MapSurfelGeometryToCuda(
        int idx, SurfelGeometry& geometry, cudaStream_t stream = 0);
...
private:
    ...
    GLShaderProgram
        m_solver_map_shader; // This shader will draw recent observation
    ...
    void drawSolverMaps(unsigned num_vertex, int vao_idx, int current_time,
        const Matrix4f& world2camera, bool with_recent_observation);

    // The workforce method for offline visualization
    void drawVisualizationMap(GLShaderProgram& shader, GLuint geometry_vao,
        unsigned num_vertex, int current_time, const Matrix4f& world2camera,
        bool with_recent_observation);

public:
    void DrawFusionMaps(
        unsigned num_vertex, int vao_idx, const Matrix4f& world2camera);
    void DrawSolverMapsConfidentObservation(unsigned num_vertex, int vao_idx,
        int current_time, const Matrix4f& world2camera);
    void DrawSolverMapsWithRecentObservation(unsigned num_vertex, int vao_idx,
        int current_time, const Matrix4f& world2camera);

    // The offline visualization methods
    void SaveLiveNormalMap(unsigned num_vertex, int vao_idx, int current_time,
        const Matrix4f& world2camera, const std::string& path,
        bool with_recent = true);
    void SaveLiveAlbedoMap(unsigned num_vertex, int vao_idx, int current_time,
        const Matrix4f& world2camera, const std::string& path,
        bool with_recent = true);
    void SaveLivePhongMap(unsigned num_vertex, int vao_idx, int current_time,
        const Matrix4f& world2camera, const std::string& path,
        bool with_recent = true);
    void SaveReferenceNormalMap(unsigned num_vertex, int vao_idx,
        int current_time, const Matrix4f& world2camera, const std::string& path,
        bool with_recent = true);
    void SaveReferenceAlbedoMap(unsigned num_vertex, int vao_idx,
        int current_time, const Matrix4f& world2camera, const std::string& path,
        bool with_recent = true);
    void SaveReferencePhongMap(unsigned num_vertex, int vao_idx,
        int current_time, const Matrix4f& world2camera, const std::string& path,
        bool with_recent = true);
