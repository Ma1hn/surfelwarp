    ...
    int scaled_width, int scaled_height)
{
    ...
    glRenderbufferStorage(
        GL_RENDERBUFFER, GL_RGBA32F, scaled_width, scaled_height);
    ...
    glRenderbufferStorage(
        GL_RENDERBUFFER, GL_DEPTH_COMPONENT32F, scaled_width, scaled_height);

    // The data in index map shall be unsigned int
    glBindRenderbuffer(GL_RENDERBUFFER, index_map);
    glRenderbufferStorage(
        GL_RENDERBUFFER, GL_LUMINANCE32UI_EXT, scaled_width, scaled_height);

    // Attach the render buffer to frame buffer
    glBindFramebuffer(GL_FRAMEBUFFER, fusion_map_fbo);
    glFramebufferRenderbuffer(
        GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_RENDERBUFFER, warp_vertex_map);
    glFramebufferRenderbuffer(
        GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT1, GL_RENDERBUFFER, warp_normal_map);
    glFramebufferRenderbuffer(
        GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT2, GL_RENDERBUFFER, index_map);
    glFramebufferRenderbuffer(
        GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT3, GL_RENDERBUFFER, color_time_map);
    glFramebufferRenderbuffer(
    ...
    // Enable draw-buffers
    GLuint draw_buffers[] = {GL_COLOR_ATTACHMENT0, GL_COLOR_ATTACHMENT1,
    ...
    cudaSafeCall(cudaGraphicsGLRegisterImage(&cuda_rbo_resources[0],
        warp_vertex_map, GL_RENDERBUFFER, cudaGraphicsRegisterFlagsReadOnly));
    cudaSafeCall(cudaGraphicsGLRegisterImage(&cuda_rbo_resources[1],
        warp_normal_map, GL_RENDERBUFFER, cudaGraphicsRegisterFlagsReadOnly));
    cudaSafeCall(cudaGraphicsGLRegisterImage(&cuda_rbo_resources[2], index_map,
        GL_RENDERBUFFER, cudaGraphicsRegisterFlagsReadOnly));
    cudaSafeCall(cudaGraphicsGLRegisterImage(&cuda_rbo_resources[3],
        color_time_map, GL_RENDERBUFFER, cudaGraphicsRegisterFlagsReadOnly));
...
void surfelwarp::GLFusionMapsFrameRenderBufferObjects::mapToCuda(
    ...
    cudaTextureObject_t& index_texture, cudaTextureObject_t& color_time_texture,
    cudaStream_t stream)
{
    ...
    cudaSafeCall(cudaGraphicsSubResourceGetMappedArray(
        &(cuda_mapped_arrays[0]), cuda_rbo_resources[0], 0, 0));
    cudaSafeCall(cudaGraphicsSubResourceGetMappedArray(
        &(cuda_mapped_arrays[1]), cuda_rbo_resources[1], 0, 0));
    cudaSafeCall(cudaGraphicsSubResourceGetMappedArray(
        &(cuda_mapped_arrays[2]), cuda_rbo_resources[2], 0, 0));
    cudaSafeCall(cudaGraphicsSubResourceGetMappedArray(
        &(cuda_mapped_arrays[3]), cuda_rbo_resources[3], 0, 0));
    ...
    cudaCreateTextureObject(
        &(cuda_mapped_texture[0]), &resource_desc, &texture_desc, NULL);
    resource_desc.res.array.array = cuda_mapped_arrays[1];
    cudaCreateTextureObject(
        &(cuda_mapped_texture[1]), &resource_desc, &texture_desc, NULL);
    resource_desc.res.array.array = cuda_mapped_arrays[2];
    cudaCreateTextureObject(
        &(cuda_mapped_texture[2]), &resource_desc, &texture_desc, NULL);
    resource_desc.res.array.array = cuda_mapped_arrays[3];
    cudaCreateTextureObject(
        &(cuda_mapped_texture[3]), &resource_desc, &texture_desc, NULL);
...
void surfelwarp::GLFusionMapsFrameRenderBufferObjects::unmapFromCuda(
    cudaStream_t stream)
{
 ...
 */
void surfelwarp::GLSolverMapsFrameRenderBufferObjects::initialize(
    int width, int height)
{
    ...
    glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
        GL_RENDERBUFFER, reference_vertex_map);
    glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT1,
        GL_RENDERBUFFER, reference_normal_map);
    glFramebufferRenderbuffer(
        GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT2, GL_RENDERBUFFER, warp_vertex_map);
    glFramebufferRenderbuffer(
        GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT3, GL_RENDERBUFFER, warp_normal_map);
    glFramebufferRenderbuffer(
        GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT4, GL_RENDERBUFFER, index_map);
    glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT5,
        GL_RENDERBUFFER, normalized_rgb_map);
    glFramebufferRenderbuffer(
        GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, depth_buffer);
    ...
    cudaSafeCall(cudaGraphicsGLRegisterImage(&cuda_rbo_resources[0],
        reference_vertex_map, GL_RENDERBUFFER,
        cudaGraphicsRegisterFlagsReadOnly));
    cudaSafeCall(cudaGraphicsGLRegisterImage(&cuda_rbo_resources[1],
        reference_normal_map, GL_RENDERBUFFER,
        cudaGraphicsRegisterFlagsReadOnly));
    cudaSafeCall(cudaGraphicsGLRegisterImage(&cuda_rbo_resources[2],
        warp_vertex_map, GL_RENDERBUFFER, cudaGraphicsRegisterFlagsReadOnly));
    cudaSafeCall(cudaGraphicsGLRegisterImage(&cuda_rbo_resources[3],
        warp_normal_map, GL_RENDERBUFFER, cudaGraphicsRegisterFlagsReadOnly));
    cudaSafeCall(cudaGraphicsGLRegisterImage(&cuda_rbo_resources[4], index_map,
        GL_RENDERBUFFER, cudaGraphicsRegisterFlagsReadOnly));
    cudaSafeCall(
        cudaGraphicsGLRegisterImage(&cuda_rbo_resources[5], normalized_rgb_map,
            GL_RENDERBUFFER, cudaGraphicsRegisterFlagsReadOnly));
    cudaSafeCall(cudaGetLastError());
}
...
void surfelwarp::GLSolverMapsFrameRenderBufferObjects::mapToCuda(
    ...
    cudaTextureObject_t& normalized_rgb_texture, cudaStream_t stream)
{
    // First map the resource
    ...
    cudaSafeCall(cudaGraphicsSubResourceGetMappedArray(
        &(cuda_mapped_arrays[3]), cuda_rbo_resources[3], 0, 0));
    cudaSafeCall(cudaGraphicsSubResourceGetMappedArray(
        &(cuda_mapped_arrays[4]), cuda_rbo_resources[4], 0, 0));
    cudaSafeCall(cudaGraphicsSubResourceGetMappedArray(
        &(cuda_mapped_arrays[5]), cuda_rbo_resources[5], 0, 0));
    ...
    cudaCreateTextureObject(
        &(cuda_mapped_texture[4]), &resource_desc, &texture_desc, NULL);
    resource_desc.res.array.array = cuda_mapped_arrays[5];
    cudaCreateTextureObject(
        &(cuda_mapped_texture[5]), &resource_desc, &texture_desc, NULL);
...
void surfelwarp::GLSolverMapsFrameRenderBufferObjects::unmapFromCuda(
    cudaStream_t stream)
{
    cudaSafeCall(cudaGraphicsUnmapResources(6, cuda_rbo_resources, stream));
}

/* The buffer and methods for offline visualization frame & render buffer
 * objects
 */
void surfelwarp::GLOfflineVisualizationFrameRenderBufferObjects::initialize(
    int width, int height)
{
    // Generate the framebuffer object
    ...
    glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
        GL_RENDERBUFFER, normalized_rgba_rbo);
    glFramebufferRenderbuffer(
        GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, depth_buffer);
...
void surfelwarp::GLOfflineVisualizationFrameRenderBufferObjects::save(
    const std::string& path)
{
    ...
    glBindFramebuffer(GL_FRAMEBUFFER, visualization_map_fbo);
    glReadPixels(
        0, 0, width, height, GL_RGBA, GL_UNSIGNED_BYTE, rendered_map_cv.data);
