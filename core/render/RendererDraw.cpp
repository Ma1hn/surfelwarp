...
void surfelwarp::Renderer::initProcessingShaders()
{
    ...
    m_fusion_map_shader.Compile(
        fusion_map_vert_path.string(), fusion_map_frag_path.string());
    ...
    const auto solver_map_recent_vert_path
        = shader_path / "solver_map_sized.vert";
#else
    const auto solver_map_recent_vert_path = shader_path / "solver_map.vert";
#endif
    m_solver_map_shader.Compile(
        solver_map_recent_vert_path.string(), solver_map_frag_path.string());
}
...
void surfelwarp::Renderer::initVisualizationShaders()
{
    ...
    m_visualization_shaders.normal_map.Compile(
        geometry_vert_path.string(), normal_map_frag_path.string());
    m_visualization_shaders.phong_map.Compile(
        geometry_vert_path.string(), phong_shading_path.string());
    m_visualization_shaders.albedo_map.Compile(
        geometry_vert_path.string(), albedo_color_path.string());
}
...
void surfelwarp::Renderer::DrawFusionMaps(
    unsigned num_vertex, int vao_idx, const Matrix4f& world2camera)
{
    // Bind the shader
    ...
    m_fusion_map_shader.SetUniformVector(
        "width_height_maxdepth", m_width_height_maxdepth);
 ...
 */
void surfelwarp::Renderer::drawSolverMaps(unsigned num_vertex, int vao_idx,
    int current_time, const surfelwarp::Matrix4f& world2camera,
    bool with_recent_observation)
{
    ...
    float4 width_height_maxdepth_currtime
        = make_float4(m_width_height_maxdepth.x, m_width_height_maxdepth.y,
            m_width_height_maxdepth.z, current_time);
    m_solver_map_shader.SetUniformVector(
        "width_height_maxdepth_currtime", width_height_maxdepth_currtime);

    // The time threshold depend on input
    float2 confid_time_threshold
        = make_float2(Constants::kStableSurfelConfidenceThreshold,
            Constants::kRenderingRecentTimeThreshold);
    if (!with_recent_observation) {
        confid_time_threshold.y
            = -1.0f; // Do not pass any surfel due to recent observed
    }

    m_solver_map_shader.SetUniformVector(
        "confid_time_threshold", confid_time_threshold);
...
void surfelwarp::Renderer::DrawSolverMapsConfidentObservation(
    unsigned num_vertex, int vao_idx,
...
void surfelwarp::Renderer::DrawSolverMapsWithRecentObservation(
    unsigned num_vertex, int vao_idx, int current_time,
    const surfelwarp::Matrix4f& world2camera)
{
 ...
 */
void surfelwarp::Renderer::drawVisualizationMap(GLShaderProgram& shader,
    GLuint geometry_vao, unsigned num_vertex, int current_time,
    const Matrix4f& world2camera, bool with_recent_observation)
{
    // Bind the shader
    shader.Bind();
    ...
    if (config.isShowOnline()) {
        // online rendering
        glBindFramebuffer(GL_FRAMEBUFFER, 0);
        glViewport(100, 200, m_image_width + 100, m_image_height + 200);
    } else {
        // offline rendering
        glBindFramebuffer(
            GL_FRAMEBUFFER, m_visualization_draw_buffers.visualization_map_fbo);
        glViewport(0, 0, m_image_width, m_image_height);
    }
    ...
    // The current time of the solver maps
    const float4 width_height_maxdepth_currtime
        = make_float4(m_width_height_maxdepth.x, m_width_height_maxdepth.y,
            m_width_height_maxdepth.z, current_time);
...
void surfelwarp::Renderer::SaveLiveNormalMap(unsigned num_vertex, int vao_idx,
    int current_time, const surfelwarp::Matrix4f& world2camera,
    const std::string& path, bool with_recent)
{
    // Draw it
    const auto geometry_vao = m_live_geometry_vao[vao_idx];
    drawVisualizationMap(m_visualization_shaders.normal_map, geometry_vao,
        num_vertex, current_time, world2camera, with_recent);

    // Save it
    m_visualization_draw_buffers.save(path);
}

void surfelwarp::Renderer::SaveLiveAlbedoMap(unsigned num_vertex, int vao_idx,
    int current_time, const surfelwarp::Matrix4f& world2camera,
    const std::string& path, bool with_recent)
{
    // Draw it
    const auto geometry_vao = m_live_geometry_vao[vao_idx];
    drawVisualizationMap(m_visualization_shaders.albedo_map, geometry_vao,
        num_vertex, current_time, world2camera, with_recent);

    // Save it
    // if online show is enabled, do not save the image, so if want to save the
    // image, disable the online show, and enable offline show
    if (!ConfigParser::Instance().isShowOnline()) {
        m_visualization_draw_buffers.save(path);
    }
}

void surfelwarp::Renderer::SaveLivePhongMap(unsigned num_vertex, int vao_idx,
    int current_time, const surfelwarp::Matrix4f& world2camera,
    const std::string& path, bool with_recent)
{
    // Draw it
    const auto geometry_vao = m_live_geometry_vao[vao_idx];
    drawVisualizationMap(m_visualization_shaders.phong_map, geometry_vao,
        num_vertex, current_time, world2camera, with_recent);
...
void surfelwarp::Renderer::SaveReferenceNormalMap(unsigned num_vertex,
    int vao_idx, int current_time, const Matrix4f& world2camera,
    const std::string& path, bool with_recent)
{
...
void surfelwarp::Renderer::SaveReferenceAlbedoMap(unsigned num_vertex,
    int vao_idx, int current_time, const Matrix4f& world2camera,
    const std::string& path, bool with_recent)
{
    const auto geometry_vao = m_reference_geometry_vao[vao_idx];
...
void surfelwarp::Renderer::SaveReferencePhongMap(unsigned num_vertex,
    int vao_idx, int current_time, const Matrix4f& world2camera,
    const std::string& path, bool with_recent)
{
    const auto geometry_vao = m_reference_geometry_vao[vao_idx];
