...
void surfelwarp::GLSurfelGeometryVBO::mapToCuda(
    surfelwarp::SurfelGeometry& geometry, cudaStream_t stream)
{
    ...
    cudaSafeCall(cudaGraphicsResourceGetMappedPointer(
        &dptr, &buffer_size, cuda_vbo_resources[0]));
    geometry.m_reference_vertex_confid = DeviceSliceBufferArray<float4>(
        (float4*)dptr, buffer_size / sizeof(float4), num_valid_surfels);

    // reference normal-radius
    cudaSafeCall(cudaGraphicsResourceGetMappedPointer(
        &dptr, &buffer_size, cuda_vbo_resources[1]));
    geometry.m_reference_normal_radius = DeviceSliceBufferArray<float4>(
        (float4*)dptr, buffer_size / sizeof(float4), num_valid_surfels);

    // live vertex confidence
    cudaSafeCall(cudaGraphicsResourceGetMappedPointer(
        &dptr, &buffer_size, cuda_vbo_resources[2]));
    geometry.m_live_vertex_confid = DeviceSliceBufferArray<float4>(
        (float4*)dptr, buffer_size / sizeof(float4), num_valid_surfels);

    // live normal-radius
    cudaSafeCall(cudaGraphicsResourceGetMappedPointer(
        &dptr, &buffer_size, cuda_vbo_resources[3]));
    geometry.m_live_normal_radius = DeviceSliceBufferArray<float4>(
        (float4*)dptr, buffer_size / sizeof(float4), num_valid_surfels);

    // color time
    cudaSafeCall(cudaGraphicsResourceGetMappedPointer(
        &dptr, &buffer_size, cuda_vbo_resources[4]));
    geometry.m_color_time = DeviceSliceBufferArray<float4>(
        (float4*)dptr, buffer_size / sizeof(float4), num_valid_surfels);
}
...
void surfelwarp::initializeGLSurfelGeometry(
    surfelwarp::GLSurfelGeometryVBO& vbo)
{
    // Generate five vbo
    ...
    // Bind each vbo and allocate memory to each vbo
    glBindBuffer(GL_ARRAY_BUFFER, vbo.reference_vertex_confid);
    glBufferData(GL_ARRAY_BUFFER, 4 * sizeof(float) * Constants::kMaxNumSurfels,
    ...
    // Register buffer to cuda, thus cuda can access these vbo data
    cudaSafeCall(cudaGraphicsGLRegisterBuffer(&(vbo.cuda_vbo_resources[0]),
        vbo.reference_vertex_confid, cudaGraphicsRegisterFlagsNone));
    cudaSafeCall(cudaGraphicsGLRegisterBuffer(&(vbo.cuda_vbo_resources[1]),
        vbo.reference_normal_radius, cudaGraphicsRegisterFlagsNone));
    cudaSafeCall(cudaGraphicsGLRegisterBuffer(&(vbo.cuda_vbo_resources[2]),
        vbo.live_vertex_confid, cudaGraphicsRegisterFlagsNone));
    cudaSafeCall(cudaGraphicsGLRegisterBuffer(&(vbo.cuda_vbo_resources[3]),
        vbo.live_normal_radius, cudaGraphicsRegisterFlagsNone));
    cudaSafeCall(cudaGraphicsGLRegisterBuffer(&(vbo.cuda_vbo_resources[4]),
        vbo.color_time, cudaGraphicsRegisterFlagsNone));
