...
__global__ void computeDensityMapJacobian(
    ...
    const ushort4* density_term_knn, const float4* density_term_knn_weight,
    // The warp field information
    const DualQuaternion* device_warp_field, const Intrinsic intrinsic,
    const mat34 world2camera,
    // Output
    TwistGradientOfScalarCost* gradient, float* residual_array)
{
    const auto idx = threadIdx.x + blockDim.x * blockIdx.x;
    if (idx >= density_term_pixels.Size())
        return; // The function does not involve warp/block sync
    ...
    const float4 reference_vertex
        = tex2D<float4>(reference_vertex_map, pixel.x, pixel.y);
    const float4 rendered_rgb
        = tex2D<float4>(rendered_rgb_map, pixel.x, pixel.y);
    ...
    computeImageDensityJacobainAndResidualHuberWeight(density_map,
        density_gradient_map, width, height, reference_vertex, geometry_density,
        knn, knn_weight, device_warp_field, intrinsic, world2camera,
        twist_graident, residual, d_density_map_cutoff);
#else
    computeImageDensityJacobainAndResidual(density_map, density_gradient_map,
        width, height, reference_vertex, geometry_density, knn, knn_weight,
        device_warp_field, intrinsic, world2camera, twist_graident, residual);
#endif
...
{
    const auto idx = threadIdx.x + blockDim.x * blockIdx.x;
    if (idx >= foreground_term_pixels.Size())
        return; // The function does not involve warp/block sync
    ...
    const float4 reference_vertex
        = tex2D<float4>(reference_vertex_map, pixel.x, pixel.y);
    const float geometry_density = 0.0f; // An occupied pixel should be marked
                                         // as 0 on filter foreground mask

    // Compute the jacobian
    TwistGradientOfScalarCost twist_graident;
    float residual;
#if defined(USE_IMAGE_HUBER_WEIGHT)
    computeImageDensityJacobainAndResidualHuberWeight(filter_foreground_mask,
        foreground_gradient_map, width, height, reference_vertex,
        geometry_density, knn, knn_weight, device_warp_field, intrinsic,
        world2camera, twist_graident, residual, d_foreground_cutoff);
#else
    computeImageDensityJacobainAndResidual(filter_foreground_mask,
        foreground_gradient_map, width, height, reference_vertex,
        geometry_density, knn, knn_weight, device_warp_field, intrinsic,
        world2camera, twist_graident, residual);
#endif
...
void surfelwarp::DensityForegroundMapHandler::computeDensityMapTwistGradient(
    cudaStream_t stream)
{
    ...
    device::computeDensityMapJacobian<<<grid, blk, 0, stream>>>(
        ...
        m_geometry_maps.normalized_rgb_map, m_depth_observation.density_map,
        m_depth_observation.density_gradient_map, m_image_width, m_image_height,
        m_potential_pixels_knn.pixels, m_potential_pixels_knn.node_knn.RawPtr(),
        m_potential_pixels_knn.knn_weight.RawPtr(), m_node_se3,
        m_project_intrinsic, m_world2camera, m_color_twist_gradient.Ptr(),
...
void surfelwarp::DensityForegroundMapHandler::
    computeForegroundMaskTwistGradient(cudaStream_t stream)
{
    // Correct the size of output
    m_foreground_residual.ResizeArrayOrException(
        m_valid_mask_pixel.ArraySize());
    m_foreground_twist_gradient.ResizeArrayOrException(
        m_valid_mask_pixel.ArraySize());
    ...
    device::computeForegroundMaskJacobian<<<grid, blk, 0, stream>>>(
        ...
        m_depth_observation.foreground_mask_gradient_map, m_image_width,
        m_image_height, m_valid_mask_pixel.ArrayView(),
        m_valid_mask_pixel_knn.Ptr(), m_valid_mask_pixel_knn_weight.Ptr(),
        m_node_se3, m_project_intrinsic, m_world2camera,
        m_foreground_twist_gradient.Ptr(), m_foreground_residual.Ptr());
...
void surfelwarp::DensityForegroundMapHandler::ComputeTwistGradient(
    cudaStream_t color_stream, cudaStream_t foreground_stream)
{
    ...
    ForegroundMaskTerm2Jacobian& foreground_term2jacobian)
{
    ...
    density_term2jacobian.twist_gradient_array
        = m_color_twist_gradient.ArrayView();
    density_term2jacobian.check_size();

    foreground_term2jacobian.knn_array = m_valid_mask_pixel_knn.ArrayView();
    foreground_term2jacobian.knn_weight_array
        = m_valid_mask_pixel_knn_weight.ArrayView();
    foreground_term2jacobian.residual_array = m_foreground_residual.ArrayView();
    foreground_term2jacobian.twist_gradient_array
        = m_foreground_twist_gradient.ArrayView();
