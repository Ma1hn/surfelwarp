...
void surfelwarp::WarpFieldUpdater::UpdateWarpFieldFromUncoveredCandidate(
    ...
    const std::vector<float4>& node_candidate, cudaStream_t stream)
{
    ...
    for (auto i = 0; i < node_candidate.size(); i++) {
        const float4 point = make_float4(node_candidate[i].x,
            node_candidate[i].y, node_candidate[i].z, 1.0f);
    ...
    for (auto i = 0; i < added_node_candidates.size(); i++) {
        ...
        ushort4 knn;
        float4 knn_weight;
        const bool valid = ComputeSE3AtPointHost(
            warp_field, added_node_candidates[i], dq, knn, knn_weight);
        if (valid) {
    ...
    // Only copy the added part to device
    {
        const auto node_offset
            = warp_field.m_reference_node_coords.HostArraySize();
        ushort4* added_knn_dev = warp_field.m_node_knn.Ptr() + node_offset;
        cudaSafeCall(cudaMemcpyAsync(added_knn_dev, h_added_knn.data(),
            sizeof(ushort4) * h_added_knn.size(), cudaMemcpyHostToDevice,
            stream));
        float4* added_knnweight_dev
            = warp_field.m_node_knn_weight.Ptr() + node_offset;
        cudaSafeCall(cudaMemcpyAsync(added_knnweight_dev,
            h_added_knnweight.data(), sizeof(float4) * h_added_knn.size(),
            cudaMemcpyHostToDevice, stream));
        warp_field.m_node_knn.ResizeArrayOrException(
            warp_field.m_node_se3.HostArraySize());
        warp_field.m_node_knn_weight.ResizeArrayOrException(
            warp_field.m_node_se3.HostArraySize());
    }
    ...
    warp_field.m_live_node_coords.ResizeArrayOrException(
        warp_field.m_node_se3.HostArraySize());
...
void surfelwarp::WarpFieldUpdater::CheckUpdatedWarpField(
    const WarpField& warp_field, const std::vector<ushort4>& h_added_knn,
    const std::vector<float4>& h_added_knnweight)
{
    ...
    const float4* skinning_from_vertex
        = warp_field.m_reference_node_coords.DevicePtr();
    const auto skinning_from_size
        = warp_field.m_reference_node_coords.DeviceArraySize()
          - h_added_knn.size();
    DeviceArrayView<float4> skinning_from
        = DeviceArrayView<float4>(skinning_from_vertex, skinning_from_size);
    KNNSearch::CheckKNNSearch(skinning_from,
        warp_field.m_reference_node_coords.DeviceArrayReadOnly(),
        warp_field.m_node_knn.ArrayView());
    ...
    LOG(INFO) << "The knn at newnode offset " << newnode_offset << " is "
              << newnode_knn.x << " " << newnode_knn.y << " " << newnode_knn.z
              << " " << newnode_knn.w << " ";
    LOG(INFO) << "The knn weight at newnode offset " << newnode_offset << " is "
              << newnode_knnweight.x << " " << newnode_knnweight.y << " "
              << newnode_knnweight.z << " " << newnode_knnweight.w << " ";

    // The added knn
    LOG(INFO) << "The append knn is " << h_added_knn[0].x << " "
              << h_added_knn[0].y << " " << h_added_knn[0].z << " "
              << h_added_knn[0].w << " ";
    LOG(INFO) << "The append knn weight is " << h_added_knnweight[0].x << " "
              << h_added_knnweight[0].y << " " << h_added_knnweight[0].z << " "
              << h_added_knnweight[0].w << " ";
}

bool surfelwarp::WarpFieldUpdater::ComputeSE3AtPointHost(
    const WarpField& warp_field, const float4& point, DualQuaternion& dq,
    ushort4& knn, float4& knn_weight)
{
    // Collect knn at host
    std::vector<int> node_idx_array(
        warp_field.m_reference_node_coords.HostArray().size());
    const auto& coord_array = warp_field.m_reference_node_coords.HostArray();
    int count = 0;
    std::generate_n(node_idx_array.begin(), node_idx_array.size(),
        [&count]() -> int { return count++; });
    std::partial_sort(node_idx_array.begin(), node_idx_array.begin() + 4,
        node_idx_array.end(),
        [&coord_array, &point](const int& l, const int& r) -> bool {
            return squared_norm_xyz(coord_array[l] - point)
                   < squared_norm_xyz(coord_array[r] - point);
        });

    // The nearest neighbour is ready, now average on the dual quaternion
    DualQuaternion dq_average;
    float weight_sum = 0.0f;
    unsigned short knn_array[4];
    float knn_weight_array[4];
    dq_average.set_zero();
    for (int nn_iter = 0; nn_iter < 4; nn_iter++) {
        ...
        const float weight
            = expf(-0.5f * distance_square / d_node_radius_square);
        const DualQuaternion node_dq
            = warp_field.m_node_se3.HostArray()[neighbour];
    ...
    for (auto i = 0; i < 4; i++) {
        if (weight_sum < 1e-1f) {
            ...
            knn_weight_array[i] *= (1.0f / weight_sum);
    }
    ...
    knn_weight = make_float4(knn_weight_array[3], knn_weight_array[2],
        knn_weight_array[1], knn_weight_array[0]);
    return true;
}

void surfelwarp::WarpFieldUpdater::InitializeReferenceNodesAndSE3FromCandidates(
    WarpField& warp_field, const std::vector<float4>& node_candidate,
    cudaStream_t stream)
{
    ...
    // Collect elements
    const float sample_distance_square
        = (Constants::kNodeSamplingRadius) * (Constants::kNodeSamplingRadius);
    ...
    for (auto vert_iter = 0; vert_iter < h_candidate.size(); vert_iter++) {
        ...
        const float4 point = make_float4(
            candidate_point.x, candidate_point.y, candidate_point.z, 1.0f);
        bool is_node = true;
        for (auto node_iter = 0; node_iter < h_nodes.size(); node_iter++) {
            if (squared_norm_xyz(point - h_nodes[node_iter])
                < sample_distance_square) {
                is_node = false;
                break;
            }
        }
        ...
        if (is_node) {
            ...
            h_node_se3.emplace_back(
                DualQuaternion(Quaternion(1, 0, 0, 0), Quaternion(0, 0, 0, 0)));
        }
    }
