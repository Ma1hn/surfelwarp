...
__global__ void buildCandidateSurfelAndFiniteDiffVertexKernel(
    ...
    cudaTextureObject_t color_time_map, mat34 camera2world,
    DeviceArrayView<ushort2> candidate_pixel, const float finitediff_step,
    // The output
    float4* finitediff_vertex, float4* surfel_vertex_confid,
    float4* surfel_normal_radius, float4* surfel_color_time)
{
    const auto idx = threadIdx.x + blockIdx.x * blockDim.x;
    if (idx < candidate_pixel.Size()) {
        const ushort2 pixel = candidate_pixel[idx];
        const float4 depth_vertex_confid
            = tex2D<float4>(depth_vertex_confid_map, pixel.x, pixel.y);
        const float4 depth_normal_radius
            = tex2D<float4>(depth_normal_radius_map, pixel.x, pixel.y);
        const float4 color_time
            = tex2D<float4>(color_time_map, pixel.x, pixel.y);

        // Transform the vertex into the world frame
        const float3 vertex
            = camera2world.rot * depth_vertex_confid + camera2world.trans;
        const float3 normal = camera2world.rot * depth_normal_radius;

        // Write to finitediff vertex buffer
        const auto offset = idx * 4;
        finitediff_vertex[offset + 0] = make_float4(vertex.x, vertex.y,
            vertex.z,
            depth_vertex_confid.w); // Note that the last element is confidence
        finitediff_vertex[offset + 1]
            = make_float4(vertex.x + finitediff_step, vertex.y, vertex.z, 1.0f);
        finitediff_vertex[offset + 2]
            = make_float4(vertex.x, vertex.y + finitediff_step, vertex.z, 1.0f);
        finitediff_vertex[offset + 3]
            = make_float4(vertex.x, vertex.y, vertex.z + finitediff_step, 1.0f);

        // Write to surfel array
        surfel_vertex_confid[idx]
            = make_float4(vertex.x, vertex.y, vertex.z, depth_vertex_confid.w);
        surfel_normal_radius[idx]
            = make_float4(normal.x, normal.y, normal.z, depth_normal_radius.w);
...
struct SurfelCandidateFilterDevice
{
    ...
    __host__ __device__ __forceinline__ float min_distance2node_squared(
        const float4& vertex, const ushort4& knn) const
    {
        // The first knn
        ...
        const ushort4& knn) const
    {
        ...
        for (auto i = 0; i < 4; i++) {
            for (auto j = i + 1; j < 4; j++) {
                live_pairwise_distance[shift] = squared_norm_xyz(
                    warpfield_input.live_node_coords[knn_array[i]]
                    - warpfield_input.live_node_coords[knn_array[j]]);
                canonical_pairwise_distance[shift] = squared_norm_xyz(
                    warpfield_input.reference_node_coords[knn_array[i]]
                    - warpfield_input.reference_node_coords[knn_array[j]]);
                shift++;
            }
        }
        ...
        for (auto i = 0; i < 6; i++) {
            if (live_pairwise_distance[i]
                < 0.64f * canonical_pairwise_distance[i])
                consistent_skinning = false;
        }
    ...
    __device__ __forceinline__ void processFiltering() const
    {
        ...
        if (offset >= vertex_input.vertex_finitediff_array.Size())
            return;
        ...
        const float4 vertex_knnweight
            = vertex_input.vertex_finitediff_knnweight[offset];

        // The written marker
        unsigned candidate_valid = 1;

        // Check distance
        if (min_distance2node_squared(vertex, vertex_knn)
            >= 4 * d_node_radius_square)
            candidate_valid = 0;

        // Check the consistent of skinning
        if (!is_skinning_consistent(vertex_knn))
            candidate_valid = 0;
        ...
        // Check collision
        {
            ...
            for (auto i = 0; i < 3; i++) {
                finitediff_vertex[i]
                    = vertex_input.vertex_finitediff_array[offset + 1 + i];
                finitediff_knn[i]
                    = vertex_input.vertex_finitediff_knn[offset + 1 + i];
                finitediff_weight[i]
                    = vertex_input.vertex_finitediff_knnweight[offset + 1 + i];
            }

            // Check it
            const bool compression
                = is_compressive_mapped(vertex, vertex_knn, vertex_knnweight,
                    finitediff_vertex, finitediff_knn, finitediff_weight,
                    warpfield_input.node_se3, vertex_input.finitediff_step);
            if (compression)
                candidate_valid = 0;
        }
...
/* The method to build vertex. Using either indicator or pixels. The indicator
 * will case sync
 */
void surfelwarp::AppendSurfelProcessor::BuildSurfelAndFiniteDiffVertex(
    cudaStream_t stream)
{
    ...
    m_candidate_vertex_finite_diff.ResizeArrayOrException(
    ...
    device::
        buildCandidateSurfelAndFiniteDiffVertexKernel<<<grid, blk, 0, stream>>>(
            m_observation.vertex_confid_map, m_observation.normal_radius_map,
            m_observation.color_time_map, m_camera2world,
            m_surfel_candidate_pixel, kFiniteDiffStep,
            // The output
            m_candidate_vertex_finite_diff.Ptr(), m_surfel_vertex_confid.Ptr(),
            m_surfel_normal_radius.Ptr(), m_surfel_color_time.Ptr());
    ...
    // SURFELWARP_CHECK(h_candidate_finitediff_vec.size() ==
    // m_surfel_candidate_pixel.Size() * 4);
#endif
}

void surfelwarp::AppendSurfelProcessor::SkinningFiniteDifferenceVertex(
    cudaStream_t stream)
{
    // Resize the array
    m_candidate_vertex_finitediff_knn.ResizeArrayOrException(
        m_candidate_vertex_finite_diff.ArraySize());
    m_candidate_vertex_finitediff_knnweight.ResizeArrayOrException(
        m_candidate_vertex_finite_diff.ArraySize());
    ...
    m_live_node_skinner->Skinning(m_candidate_vertex_finite_diff.ArrayView(),
        m_candidate_vertex_finitediff_knn.ArraySlice(),
        m_candidate_vertex_finitediff_knnweight.ArraySlice(), stream);
...
void surfelwarp::AppendSurfelProcessor::FilterCandidateSurfels(
    cudaStream_t stream)
{
    // Resize the indicator
    m_candidate_surfel_validity_indicator.ResizeArrayOrException(
        m_surfel_candidate_pixel.Size());
    ...
    if (m_surfel_knn.ArraySize() == 0)
        return;

    // Construct the filter
    device::SurfelCandidateFilterDevice filter;

    filter.warpfield_input.live_node_coords
        = m_warpfield_input.live_node_coords.RawPtr();
    filter.warpfield_input.reference_node_coords
        = m_warpfield_input.reference_node_coords.RawPtr();
    filter.warpfield_input.node_se3 = m_warpfield_input.node_se3.RawPtr();

    filter.vertex_input.vertex_finitediff_array
        = m_candidate_vertex_finite_diff.ArrayView();
    filter.vertex_input.vertex_finitediff_knn
        = m_candidate_vertex_finitediff_knn.Ptr();
    filter.vertex_input.vertex_finitediff_knnweight
        = m_candidate_vertex_finitediff_knnweight.Ptr();
    filter.vertex_input.finitediff_step = kFiniteDiffStep;

    filter.candidate_validity_indicator
        = m_candidate_surfel_validity_indicator.Ptr();
    ...
    m_candidate_surfel_validity_prefixsum.InclusiveSum(
        m_candidate_surfel_validity_indicator.ArrayView(), stream);
    ...
    // LOG(INFO) << "The number of valid appended surfels is " <<
    // numNonZeroElement(m_candidate_surfel_validity_indicator.ArrayView());
}
