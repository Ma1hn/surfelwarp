...
private:
    ...
    friend class SurfelNodeDeformer; // deform the vertex/normal given warp
                                     // field
    friend class DoubleBufferCompactor; // compact from one buffer to another in
                                        // double buffer setup
...
public:
    DeviceArrayView<float4> GetReferenceVertexConfidence() const
    {
        return m_reference_vertex_confid.ArrayView();
    }
    DeviceArrayView<float4> GetLiveVertexConfidence() const
    {
        return m_live_vertex_confid.ArrayView();
    }
    DeviceArrayView<float4> GetColorTime() const
    {
        return m_color_time.ArrayView();
    }
...
public:
     ...
     */
    DeviceArrayView<float4> ReferenceVertexArray() const
    {
        return m_reference_vertex_confid.ArrayView();
    }
    DeviceArrayView<ushort4> SurfelKNNArray() const
    {
        return m_surfel_knn.ArrayView();
    }
