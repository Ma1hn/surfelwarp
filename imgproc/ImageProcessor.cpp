...
surfelwarp::ImageProcessor::ImageProcessor(
    const surfelwarp::FetchInterface::Ptr& fetcher)
: m_image_fetcher(fetcher)
{
...
/* Interface functions
 */
surfelwarp::DeviceArrayView<surfelwarp::DepthSurfel>
surfelwarp::ImageProcessor::ProcessFirstFrameSerial(
    size_t frame_idx, cudaStream_t stream)
{
    ...
    return DeviceArrayView<DepthSurfel>(
        surfel_array.ptr(), surfel_array.size());
}

void surfelwarp::ImageProcessor::ProcessFrameSerial(
    CameraObservation& observation, size_t frame_idx, cudaStream_t stream)
{
    FetchFrame(frame_idx);
    UploadDepthImage(stream);
    UploadRawRGBImage(stream);
...
void surfelwarp::ImageProcessor::allocateFetchBuffer()
{
    ...
    cudaSafeCall(cudaMallocHost(
        &m_depth_buffer_pagelock, sizeof(unsigned short) * raw_img_size));
    cudaSafeCall(
        cudaMallocHost(&m_rgb_buffer_pagelock, sizeof(uchar4) * raw_img_size));
    cudaSafeCall(cudaMallocHost(
        &m_rgb_prev_buffer_pagelock, sizeof(uchar4) * raw_img_size));
...
void surfelwarp::ImageProcessor::allocateDepthTexture()
{
    ...
    createDepthTextureSurface(
        m_raw_img_rows, m_raw_img_cols, m_depth_raw_collect);

    // The filtered image should use clip_size
    createDepthTextureSurface(
        m_clip_img_rows, m_clip_img_cols, m_depth_filter_collect);
...
{
    cudaSafeCall(cudaMemcpyToArrayAsync(m_depth_raw_collect.d_array, 0, 0,
...
void surfelwarp::ImageProcessor::ReprojectDepthToRGB(cudaStream_t stream)
{
    ...
    cudaSafeCall(cudaMemset2DAsync(m_reprojected_buffer.ptr(),
        m_reprojected_buffer.colsBytes(), 0, m_reprojected_buffer.cols(),
        m_reprojected_buffer.rows(), stream));

    // Do reprojection
    const auto depth_raw_intrinsic_inv = inverse(m_raw_depth_intrinsic);
    reprojectDepthToRGB(m_depth_raw_collect.texture,
        m_depth_raw_collect.surface, m_reprojected_buffer, m_raw_img_rows,
        m_raw_img_cols, depth_raw_intrinsic_inv, m_raw_rgb_intrinsic,
        m_depth2rgb, stream);
}

void surfelwarp::ImageProcessor::ClipFilterDepthImage(cudaStream_t stream)
{
    clipFilterDepthImage(m_depth_raw_collect.texture, m_clip_img_rows,
        m_clip_img_cols, m_clip_near, m_clip_far,
        m_depth_filter_collect.surface, stream);
}
...
void surfelwarp::ImageProcessor::allocateRGBBuffer()
{
    ...
    createFloat4TextureSurface(
        m_clip_img_rows, m_clip_img_cols, m_clip_normalize_rgb_collect);
    createFloat4TextureSurface(
        m_clip_img_rows, m_clip_img_cols, m_clip_normalize_rgb_collect_prev);

    // The texture and surface for denstiy map, in the same size as
    // clip_normalized rgb image
    createFloat1TextureSurface(
        m_clip_img_rows, m_clip_img_cols, m_density_map_collect);
    createFloat1TextureSurface(
        m_clip_img_rows, m_clip_img_cols, m_filter_density_map_collect);
}
...
void surfelwarp::ImageProcessor::UploadRawRGBImage(cudaStream_t stream)
{
    ...
    cudaSafeCall(cudaMemcpyAsync(ptr, m_rgb_buffer_pagelock,
        sizeof(uchar3) * m_raw_img_cols * m_raw_img_rows,
        cudaMemcpyHostToDevice, stream));

    ptr = m_raw_rbg_buffer_prev.ptr();
    // This should be aligned due to the size
    cudaSafeCall(cudaMemcpyAsync(ptr, m_rgb_prev_buffer_pagelock,
        sizeof(uchar3) * m_raw_img_cols * m_raw_img_rows,
        cudaMemcpyHostToDevice, stream));
}
...
{
    clipNormalizeRGBImage(m_raw_rgb_buffer, m_clip_img_rows, m_clip_img_cols,
        m_clip_normalize_rgb_collect.surface, m_density_map_collect.surface,
        stream);

    filterDensityMap(m_density_map_collect.texture,
        m_filter_density_map_collect.surface, m_clip_img_rows, m_clip_img_cols,
        stream);

    clipNormalizeRGBImage(m_raw_rbg_buffer_prev, m_clip_img_rows,
        m_clip_img_cols, m_clip_normalize_rgb_collect_prev.surface,
...
void surfelwarp::ImageProcessor::allocateGeometryTexture()
{
    // The texture and surface of cliped & normalized rgb image
    createFloat4TextureSurface(
        m_clip_img_rows, m_clip_img_cols, m_vertex_confid_collect);
    createFloat4TextureSurface(
        m_clip_img_rows, m_clip_img_cols, m_normal_radius_collect);
}
...
void surfelwarp::ImageProcessor::BuildVertexConfigMap(cudaStream_t stream)
{
    ...
    createVertexConfigMap(m_depth_filter_collect.texture, m_clip_img_rows,
        m_clip_img_cols, clip_intrinsic_inv, m_vertex_confid_collect.surface,
        stream);
}

void surfelwarp::ImageProcessor::BuildNormalRadiusMap(cudaStream_t stream)
{
    createNormalRadiusMap(m_vertex_confid_collect.texture, m_clip_img_rows,
        m_clip_img_cols, m_normal_radius_collect.surface, stream);
}
...
{
    createFloat4TextureSurface(
        m_clip_img_rows, m_clip_img_cols, m_color_time_collect);
}
...
void surfelwarp::ImageProcessor::BuildColorTimeTexture(
    size_t frame_idx, cudaStream_t stream)
{
    const float init_time = float(frame_idx);
    createColorTimeMap(m_raw_rgb_buffer, m_clip_img_rows, m_clip_img_cols,
        init_time, m_color_time_collect.surface, stream);
}
...
void surfelwarp::ImageProcessor::CollectValidDepthSurfel(cudaStream_t stream)
{
    ...
    DeviceArray<char> valid_indicator = DeviceArray<char>(
        m_valid_depth_pixel_selector.select_indicator_buffer.ptr(), num_pixels);

    // First mark the validity of depth surfels
    markValidDepthPixel(m_depth_filter_collect.texture, m_clip_img_rows,
        m_clip_img_cols, m_valid_depth_pixel_selector.select_indicator_buffer,
        stream);
    ...
    const auto selected_surfel_size
        = m_valid_depth_pixel_selector.valid_selected_idx.size();
    ...
    collectDepthSurfel(m_vertex_confid_collect.texture,
        m_normal_radius_collect.texture, m_color_time_collect.texture,
        m_valid_depth_pixel_selector.valid_selected_idx, m_clip_img_rows,
        m_clip_img_cols, valid_surfel_array, stream);
}
...
void surfelwarp::ImageProcessor::allocateForegroundSegmentationBuffer()
{
    ...
    if (config.use_offline_foreground_segmneter()
        ...
        if (!config.use_segmentation()) {
            std::cout << "Disable segmentation" << std::endl;
        } else {
            std::cout << "Use pre-computed segmentation mask" << std::endl;
        }
        m_foreground_segmenter = std::make_shared<ForegroundSegmenterOffline>();
    } else {
        m_foreground_segmenter
            = std::make_shared<ForegroundSegmenterPermutohedral>();
        // m_foreground_segmenter =
        // std::make_shared<ForegroundSegmenterNaiveCRF>();
    }
...
void surfelwarp::ImageProcessor::SegmentForeground(
    int frame_idx, cudaStream_t stream)
{
    // Hand on the input to the segmenter
    m_foreground_segmenter->SetInputImages(m_clip_normalize_rgb_collect.texture,
        m_depth_raw_collect.texture, m_depth_filter_collect.texture, frame_idx);
...
{
    // Use gpc?
    m_feature_correspondence_finder
        = std::make_shared<PatchColliderRGBCorrespondence>();
...
void surfelwarp::ImageProcessor::FindCorrespondence(cudaStream_t stream)
{
    ...
    m_feature_correspondence_finder->SetInputImages(
        ClipNormalizedRGBTexturePrev(), ClipNormalizedRGBTexture(),
        ForegroundMask());
...
{
    createFloat2TextureSurface(m_clip_img_rows, m_clip_img_cols,
        m_foreground_mask_gradient_map_collect);
    createFloat2TextureSurface(
        m_clip_img_rows, m_clip_img_cols, m_density_gradient_map_collect);
}
...
void surfelwarp::ImageProcessor::ComputeGradientMap(cudaStream_t stream)
{
    ...
    if (kUseFilteredMap) {
        computeDensityForegroundMaskGradient(FilterForegroundMask(),
            m_filter_density_map_collect.texture, m_clip_img_rows,
            m_clip_img_cols, m_foreground_mask_gradient_map_collect.surface,
            m_density_gradient_map_collect.surface, stream);
    } else {
        computeDensityForegroundMaskGradient(FilterForegroundMask(),
            m_density_map_collect.texture, m_clip_img_rows, m_clip_img_cols,
