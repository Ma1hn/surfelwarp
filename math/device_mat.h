...
struct mat33
{
    ...
    __host__ __device__ mat33(
        const float3& _a0, const float3& _a1, const float3& _a2)
    {
        cols[0] = _a0;
        cols[1] = _a1;
        cols[2] = _a2;
    ...
    __host__ __device__ mat33 operator*(const mat33& _mat) const
    {
        mat33 mat;
        mat.m00()
            = m00() * _mat.m00() + m01() * _mat.m10() + m02() * _mat.m20();
        mat.m01()
            = m00() * _mat.m01() + m01() * _mat.m11() + m02() * _mat.m21();
        mat.m02()
            = m00() * _mat.m02() + m01() * _mat.m12() + m02() * _mat.m22();
        mat.m10()
            = m10() * _mat.m00() + m11() * _mat.m10() + m12() * _mat.m20();
        mat.m11()
            = m10() * _mat.m01() + m11() * _mat.m11() + m12() * _mat.m21();
        mat.m12()
            = m10() * _mat.m02() + m11() * _mat.m12() + m12() * _mat.m22();
        mat.m20()
            = m20() * _mat.m00() + m21() * _mat.m10() + m22() * _mat.m20();
        mat.m21()
            = m20() * _mat.m01() + m21() * _mat.m11() + m22() * _mat.m21();
        mat.m22()
            = m20() * _mat.m02() + m21() * _mat.m12() + m22() * _mat.m22();
        return mat;
    }
...
struct mat34
{
    ...
    __host__ __device__ mat34(const mat33& _rot, const float3& _trans)
    : rot(_rot)
    , trans(_trans)
    {
    }
    __host__ __device__ mat34(
        const float3& twist_rot, const float3& twist_trans);
    ...
    __host__ __device__ __forceinline__ float3 apply_inversed_se3(
        const float3& vec) const
    {
    ...
    __host__ __device__ __forceinline__ float3 apply_inversed_se3(
        const float4& vec) const
    {
        return rot.transpose_dot(
            make_float3(vec.x - trans.x, vec.y - trans.y, vec.z - trans.z));
    }
