...
public:
    ...
    __host__ void InclusiveSum(const DeviceArray<unsigned>& array_in,
        cudaStream_t stream = 0, bool debug_sync = false);
    __host__ void InclusiveSum(
        const DeviceArrayView<unsigned>& array_in, cudaStream_t stream = 0);

    __host__ void ExclusiveSum(const DeviceArray<unsigned>& array_in,
        cudaStream_t stream = 0, bool debug_sync = false);
 ...
 */
template <typename KeyT, typename ValueT> struct KeyValueSort
{
private:
    // Shared buffer
    DeviceArray<unsigned char> m_temp_storage;
...
public:
    ...
    void Sort(const DeviceArray<KeyT>& key_in,
        const DeviceArray<ValueT>& value_in, cudaStream_t stream = 0,
        int end_bit = sizeof(KeyT) * 8, bool debug_sync = false);

    void Sort(const DeviceArrayView<KeyT>& key_in,
        const DeviceArrayView<ValueT>& value_in, cudaStream_t stream = 0);
    void Sort(const DeviceArrayView<KeyT>& key_in,
        const DeviceArrayView<ValueT>& value_in, int end_bit,
        cudaStream_t stream = 0);

    // Sort key only
    void Sort(const DeviceArray<KeyT>& key_in, cudaStream_t stream = 0,
        int end_bit = sizeof(KeyT) * 8, bool debug_sync = false);
...
public:
    FlagSelection()
    {
        ...
        cudaSafeCall(
            cudaMallocHost((void**)(&m_host_num_selected), sizeof(int)));
    }
    ...
    void SelectUnsigned(const DeviceArray<char>& flags,
        const DeviceArray<unsigned>& select_from,
        DeviceArray<unsigned>& select_to_buffer,
        DeviceArray<unsigned>& valid_select_to_array, cudaStream_t stream = 0);
...
public:
    ...
    void Select(const DeviceArray<int>& key_in, cudaStream_t stream = 0,
        bool debug_sync = false);
